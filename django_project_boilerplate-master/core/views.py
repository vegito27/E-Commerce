...
from .forms  import CheckoutForm,PaymentForm ,Coupon,RefundForm
import stripe
import random
import string
    ...
    def get(self,*args,**kwargs):
        try:
            ...
            billing_address_qs=Address.objects.filter(user=self.request.user,
                                                       address_type='B',
                                                       default=True ) 
            print(billing_address_qs)
            if billing_address_qs.exists():
                context.update({'default_billing_address':billing_address_qs[0]})    
                        
            return render(self.request,"newcheckout.html",context)
        
        except ObjectDoesNotExist:
            messages.info(self.request,"You do not have active requests")
    ...
    def post(self,*args,**kwargs):
        ...
        try:
            ...
            if form.is_valid():
                ...
                if use_default_shipping:
                    print('using the  shipping address')
                    ...
                    if is_valid_form([shipping_address,shipping_address2,shipping_country,shipping_zip]):
                        ...
                        shipping_address=Address( user=self.request.user, street_address=shipping_address, apartment_address=shipping_address2,
                ...
                if same_billing_address:
                    ...
                    if is_valid_form([billing_address,billing_address2,billing_country,billing_zip]):
                    # same_billing_address=form.cleaned_data.get('same_billing_address')
                    # save_info=form.cleaned_data.get('save_info')
...
#             payment=Payment()
#             payment.stripe_chrge_id=charge['id']
...
#             messages.warning(self.request,"Something went wrong.You were not charged.Please try again later")
#             return redirect("/")
     
#         except Exception as e:
#             messages.error(self.request,"A serious Error occured.We have been modified.")
#             return redirect("/")
    ...
    def get(self, *args, **kwargs):
        ...
        if order.billing_address:
            context = {
                'order': order,
                'DISPLAY_COUPON_FORM': False,
                'STRIPE_PUBLIC_KEY' : settings.SECRET_KEY
            }
            userprofile = self.request.user.userprofile
...
def add_to_cart(request,slug):
    ...
    if order_qs.exists():
        ...
        order.items.add(order_item)
        messages.info(request,"This message was added to your cart.") 
        return redirect("core:order-summary")
...
#                 order.save()
#                 messages.success(request,"Successfully addes Coupon")
#                 return redirect("core:checkout")
    ...
    def post(self,*args,**kwargs):
        ...
        if form.is_valid():         
            try:
                ...
                order.save()
                messages.success(self.request,"Successfully addes Coupon")
                return redirect("core:checkout")
